...
/**
 * Die eingebauten Trackingwerte.
 *
 * `sort` landet in `metrics.sort_order` – einer Spalte, die auf dem Server
 * `integer` ist. Ein Wert mit Nachkommastelle bringt dort den Abgleich der
 * ganzen Tabelle zum Stehen (siehe core/ganzzahlen.ts und Migration 9), und
 * genau das ist mit „Ballaststoffe: 23.5" schon einmal passiert. Deshalb ist
 * die Liste hier ausführbar geprüft, nicht nur gut gemeint:
 * `tests/sortierwerte.test.ts` rechnet jeden Wert nach.
 */
export const METRICS: Array<{
  ...
  { key: 'fiber_g', name: 'Ballaststoffe', group: 'nutrition', unit: 'g', type: 'integer', decimals: 0, agg: 'sum', dir: 'higher_better', target: { value: 30, minus: 10, plus: 20, hardMin: 10 }, color: 'var(--series-6)', sort: 24 },
  { key: 'water_l', name: 'Wasser', group: 'nutrition', unit: 'l', type: 'number', decimals: 1, agg: 'sum', dir: 'higher_better', target: { value: 3.2, minus: 0.2, plus: 1.5, hardMin: 1.5 }, color: 'var(--series-1)', sort: 25 },
