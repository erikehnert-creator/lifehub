...
async function ruf(
  ...
  const res = await fetch(`${basisUrl(settings.sync_url)}/${pfad}`, {
    method: 'POST',
    headers: {
      apikey: key,
  ...
  if (!res.ok) {
    ...
    const code = String(daten?.error ?? `http_${res.status}`)
    const detail = daten?.detail ? ` (${daten.detail})` : ''
