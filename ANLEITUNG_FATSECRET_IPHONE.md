...
| **„Invalid signature"** | Die Signatur passt nicht zu dem, was gesendet wurde. Am 13.09.2026 lag es an der HTTP-Methode (`request_token` verlangt POST, gesendet wurde GET) – **das ist behoben**. Tritt es erneut auf, ist meist das Consumer Secret falsch oder die Uhr des Servers geht falsch | Zuerst C.4 mit frisch kopiertem Secret wiederholen. Hilft das nicht, C.5 erneut ausführen – die Edge Function muss nach jeder Code-Änderung neu veröffentlicht werden |
